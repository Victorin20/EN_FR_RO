...
void my_keyboard(string& traduction, int* finish_input_word)
{
	...
	if (Keyboard::isKeyPressed(Keyboard::BackSpace)) { while (Keyboard::isKeyPressed(Keyboard::BackSpace)) {}delete_key = true; }
	...
	if (delete_key && traduction.size() >= 1)
	{
		string straduction = "";
	...
	else if (delete_key && traduction.size() == 0)delete_key = false;
